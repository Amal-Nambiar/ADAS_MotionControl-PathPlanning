...
class AStar:
    ...
    def __init__(self, grid_map, start, goal,heuristic_type):
    ...
    def calc_heuristic(self, x, y):
        if self.heuristic_type == "Manhattan":
            ...
            return np.hypot(x - self.goal[0], y - self.goal[1])        
    ...
    def plan(self):
        ...
        start_h    = self.calc_heuristic(start_node.x,start_node.y)
        heapq.heappush(self.heap, (start_node.cost + start_h, start_idx))        
        self.goal_node = None
        ...
        while self.heap:
            ...
            for dx, dy, move_cost in self.motion:
                ...
                if self.grid_map.is_occupied(nx, ny,2):
                    continue
                    
                new_g_cost = current.cost + move_cost
                neighbour_idx = self.calc_index(nx, ny)
                
                if neighbour_idx in self.close_set:
                    continue

                h_cost  = self.calc_heuristic(nx, ny)
                f_priority =  new_g_cost + h_cost
                    
                if neighbour_idx not in self.open_set:
                    neighbour = Node(x=nx, y=ny, cost=new_g_cost, parent_index=current_id)
                    self.open_set[neighbour_idx] = neighbour
                    heapq.heappush(self.heap, (f_priority, neighbour_idx))
                else:
                    if self.open_set[neighbour_idx].cost > new_g_cost:
                        self.open_set[neighbour_idx].cost = new_g_cost
